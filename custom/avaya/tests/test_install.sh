...
OUTPUT=$(sh "$INSTALLER" --dry-run --email admin@example.invalid \
  --server-ip 192.0.2.10 --revision "$REVISION" --sha256 "$CHECKSUM")
printf '%s\n' "$OUTPUT" | grep -F "revision=$REVISION" >/dev/null ||
...
if sh "$INSTALLER" --dry-run --email admin@example.invalid \
  --server-ip 999.0.2.10 --revision "$REVISION" --sha256 "$CHECKSUM" >/dev/null 2>&1; then
  fail 'invalid server IP was accepted'
fi
if sh "$INSTALLER" --dry-run --email invalid-email \
  --server-ip 192.0.2.10 --revision "$REVISION" --sha256 "$CHECKSUM" >/dev/null 2>&1; then
...
if sh "$INSTALLER" --dry-run --email admin@example.invalid \
  --server-ip 192.0.2.10 --revision "$REVISION" --sha256 deadbeef >/dev/null 2>&1; then
...
if sh "$INSTALLER" --dry-run --email admin@example.invalid \
  ...
  fail 'movable tag revision was accepted'
fi
